...
export function removeStorigeItem(key:string){
...
export function removeCookieItem(key: string){
  ...
  return (document.cookie = key + "=" + '' + ";" + expires + ";path=/");
}
    ...
    if(values[name] == undefined) continue;
      ...
      if(name == "files"){
          for(let i = 0; i< values[name].fileList.length; i++){
              formData.append(`${name}[${i}]`, values[name].fileList[i].originFileObj)
          }
          continue;
      }
...
export function getLang(){
  return getCookie("lang") || getStorageItem("lang")
}